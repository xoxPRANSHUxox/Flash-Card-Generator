...
// ******************************* Handles are here *******************************************************//
  ...
  const addMoreCard = (e) => {
    ...
    formik.setFieldValue("cards", [...formik.values.cards, { term: "", definition: "", image: null }]);
  };
...
//  ***************************************** function return ***********************************************//
  ...
  return (
    <div>
      ...
      >
        <div className="w-full flex flex-col h-auto">
          <div className="flex flex-col flex-wrap">
            ...
            <div className="mx-4 sm:mx-12 my-2 flex items-center">
              ...
              {formik.touched.groupName && formik.errors.groupName ? (
                <div className="text-red-500 font-bold ml-4">{formik.errors.groupName}</div>
              ) : null}
            </div>
          ...
          {formik.touched.description && formik.errors.description ? (
            <div className="text-red-600 font-bold ml-12 mb-4">{formik.errors.description}</div>
          ) : null}
          ...
          <div key={index}>
            <div className="flex flex-wrap justify-start items-center w-full my-8 sm:flex-row sm:justify-center sm:mx-0 mx-4">
              ...
              <div className="flex flex-row">
                <label htmlFor={`enterTerm-${index}`} className="font-semibold text-lg text-[#1F1717] my-2">
                  Enter Term*
                </label>
                ...
                <div>
                  <input
                    ...
                    onBlur={formik.handleBlur}
                  />
                  {formik.touched.cards?.[index]?.term && formik.errors.cards?.[index]?.term && (
                    <div className="text-red-500 font-bold ml-4">{formik.errors.cards[index].term}</div>
                  )}
                </div>
              </div>

              <div className="flex flex-row">
                <label htmlFor={`definition-${index}`} className="font-semibold text-lg text-[#1F1717] my-2">
                  Enter Definition*
                </label>
                ...
                <div>
                  <input
                    ...
                    onBlur={formik.handleBlur}
                  />
                  {formik.touched.cards?.[index]?.definition && formik.errors.cards?.[index]?.definition && (
                    <div className="text-red-600 font-bold ml-4">{formik.errors.cards[index].definition}</div>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="my-4 mx-12 flex items-center">
              {!card.image ? (
                ...
                <>
                  <img src={card.image} alt="group" width={100} height={100} />
                  <DeleteIcon onClick={() => deleteImage(index)} className="cursor-pointer ml-2" />
                </>
              )}
              <input
              ...
              {formik.values.cards.length > 1 && (
                <DeleteIcon onClick={() => deleteCard(index)} className="cursor-pointer ml-2" />
              )}
            </div>

            {index === formik.values.cards.length - 1 && (
              <button className="px-4 my-2 mx-8 py-1 font-semibold text-md rounded" onClick={addMoreCard}>
                + Add More
              </button>
            )}
          </div>
      ...
      <button className="bg-[#ce5a67] rounded w-fit px-4 py-2 m-auto text-white mb-12" type="submit">
        Create
      </button>
    </div>
